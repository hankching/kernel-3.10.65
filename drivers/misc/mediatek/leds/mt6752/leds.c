...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	switch(pmic_type){
		case MT65XX_LED_PMIC_NLED_ISINK0:
			...
			mt6325_upmu_set_isink_ch0_step(ISINK_0);//4mA
		...
		case MT65XX_LED_PMIC_NLED_ISINK2:
			...
			mt6325_upmu_set_isink_ch2_step(ISINK_0);//4mA
		...
		case MT65XX_LED_PMIC_NLED_ISINK3:
			...
			mt6325_upmu_set_isink_ch3_mode(PMIC_PWM_0);
			mt6325_upmu_set_isink_ch3_step(ISINK_0);//4mA
	...
	*/	
	...
	switch (cust->mode) {
		...
		case MT65XX_LED_MODE_GPIO:
			...
			return brightness_set_gpio(cust->data, level);//modify by shenwuyi add torch control node
