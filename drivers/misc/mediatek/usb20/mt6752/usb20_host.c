...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
    ...
    if(is_on){
        ...
        bq24296_set_boost_lim(0x0); //1.0A on VBUS
...
{
#if !(defined(SWITCH_CHARGER) || defined(FPGA_PLATFORM)) && !defined(CONFIG_MTK_BQ24296_SUPPORT)
	#ifdef CONFIG_OF
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	if (mtk_musb->is_host) {
		//---add start  //songjiangchao,2014-10-31,add according to MTK
		if (mtk_musb->is_active) {
			DBG(0, "add log is_active=%d\n", mtk_musb->is_active);
			musb_stop(mtk_musb);
			mtk_musb ->is_host = musb_is_host(); //songjiangchao,2014-11-03,add according to MTK
		}
		//--add end //songjiangchao,2014-10-31,add according to MTK
