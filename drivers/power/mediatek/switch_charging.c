...
void check_battery_temperature(void)
{
	...
	else if((BMT_status.temperature > MIN_CHARGE_TEMPERATURE )&&(BMT_status.temperature<= CHARGE_TMEPERATURE_15)) {
		g_tempStatus = E_BAT_T_B_0_15;
	}
	else if((BMT_status.temperature > CHARGE_TMEPERATURE_15)&&(BMT_status.temperature <= CHARGE_TMEPERATURE_45)) {
		g_tempStatus = E_BAT_T_B_15_45;
	}
	else if((BMT_status.temperature > CHARGE_TMEPERATURE_45)&&( BMT_status.temperature <= MAX_CHARGE_TEMPERATURE)) {
		g_tempStatus = E_BAT_T_B_45_50;
	}
      ...
      switch(g_tempStatus)
	        {
	           case E_BAT_T_L_0:
  			        ...
  			        break;
  		       case E_BAT_T_B_0_15:
				   	   g_temp_input_CC_value = AC_CHARGER_CURRENT_INPUT_02C;
				   	   g_temp_CC_value = AC_CHARGER_CURRENT_02C;
  			       break;
  		       case E_BAT_T_B_15_45:
					   g_temp_input_CC_value = AC_CHARGER_CURRENT_INPUT_05C;
				   	   g_temp_CC_value = AC_CHARGER_CURRENT_05C;
  			        break;
  		       case E_BAT_T_B_45_50:
			           g_temp_input_CC_value = AC_CHARGER_CURRENT_INPUT_05C;
				   	   g_temp_CC_value = AC_CHARGER_CURRENT_05C;
  			        break;
  		       default:break;
...
#if defined(BATTERY_TEMP_CHARGING_CURRENT)
    if(( status == KAL_TRUE)||((BMT_status.bat_temp_status == E_BAT_T_B_45_50)&&(BMT_status.bat_vol > BATTERY_VOLT_04_100000_V / 1000)))
#else
	if ( status == KAL_TRUE) 
#endif
	{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
			...
			/*Set CV Voltage */
...
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
				   #ifdef BATTERY_TEMP_CHARGING_CURRENT
				     if (BMT_status.bat_temp_status == E_BAT_T_B_45_50)
				     	{
				     	  cv_value = 4112000;
				     	}
					 else
		          ...
		          battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE,&cv_value);
                   //cv_voltage = BATTERY_VOLT_04_340000_V;
                  #endif
#else
			cv_voltage = BATTERY_VOLT_04_200000_V;
